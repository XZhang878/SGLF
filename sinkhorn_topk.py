...
def soft_topk(scores, ks, max_iter=10, tau=1., nrows=None, ncols=None, return_prob=True):
    r"""
    ...
    if nrows is None:
        nrows = [scores.shape[1] for _ in range(batch_size)]
    if ncols is None:
        ncols = [scores.shape[2] for _ in range(batch_size)]
    ...
    col_prob[:, 1] += ks
    col_prob[:, 0] += torch.as_tensor(nrows) * torch.as_tensor(ncols) - ks
...
class Sinkhorn_m(nn.Module):
    ...
    def forward_log(self, s, row_prob, col_prob, nrows=None, ncols=None, dummy_row=True):
        ...
        if self.batched_operation:
            ...
            ret_log_s = torch.full((batch_size, torch.as_tensor(nrows).max() * torch.as_tensor(ncols).max(), 2), -float('inf'), device=s[0].device, dtype=s[0].dtype)
